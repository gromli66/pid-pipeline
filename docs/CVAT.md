...
**create-task** — идемпотентен: если `cvat_task_id` уже есть, возвращает существующий. Создаёт project через `get_or_create_project()` (по `cvat_project_name` из конфига). Labels строит `create_labels_from_config()` (`app/services/cvat_client.py`) — **единственный источник списка меток**, его же зовёт воркер (`worker/tasks/detection.py`). Имена меток — отображаемые названия (`display_labels`), порядок — алфавитный по ним: CVAT метки не сортирует ни в API, ни в UI, так что порядок показа разметчику задаётся порядком создания. Цвет метки — из блока `class_colors` (ключ — английское имя класса): без него цвет назначает сам CVAT, и раскраска классов меняется от проекта к проекту. Цвет применяется при **создании** метки — существующие метки не перекрашиваются ни при `get_or_create_project()`, ни при `ensure_project_labels()`; снять цвета с готового проекта можно скриптом `tools/cvat_dump_label_colors.py`.
