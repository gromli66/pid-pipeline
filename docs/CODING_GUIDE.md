...
**Закрытие шага — всегда в `finally`.** Между `execute()` и `finalize()` мутации идут по графу **на месте**, поэтому падение посреди работы оставляет оператора с изменённым холстом и без Ctrl+Z, а `undo_mgr.revision` не растёт — вкладка считает по нему `has_unsaved_changes()` и закрывается без вопроса. В тот же `finally` идёт и перерисовка: иначе сцена показывает прежние координаты поверх уже сдвинутой модели, и на сервер уходит не то, что видел оператор.

```python
cmd.execute()
try:
    engine(...)                      # мутирует граф НА МЕСТЕ
finally:
    editor._redraw_all()
    model.rebuild_edge_data_index()
    cmd.finalize()
    undo_mgr.push_executed(cmd)
```

Так сделаны `smooth_canvas` (пункт 1.1), `auto_fix` (пункт 1.x7) и `apply_resize` (пункт 1.x8 — кнопка «Применить» панели «Размеры»; там за `try` стоят четыре мутатора подряд: `_apply_sizes_from_base`, `_spread_overlaps`, `_reseat_after_resize` и тот же `auto_fix_graph`). Граница: падение самой перерисовки шаг всё равно не закроет.

⚠ Форма повторилась трижды подряд — при ревью новой команды-на-месте это первое, что стоит проверить.
