...
cvat:
  project_name: "P&ID Термогидравлика (RU)"
```
...
| `cvat.project_name` | str | Имя проекта в CVAT. Проект ищется по **точному** совпадению имени и создаётся автоматически, если не найден (`get_or_create_project`) |

> **Смена этого имени заводит НОВЫЙ проект в CVAT.** Так и было сделано при переходе на русские
> метки: `«P&ID Термогидравлика»` → `«P&ID Термогидравлика (RU)»`. Старый проект остаётся
> нетронутым, созданные в нём задачи продолжают работать — возврат аннотаций узнаёт и английские
> имена (см. [CVAT.md §7](CVAT.md)). Менять имя на уже существующий проект с другим набором
> меток нельзя: `ensure_project_labels()` дольёт туда недостающие метки, не удаляя чужие.

### 3.2 `classes`

Реестр классов оборудования. ID 1-based — это **канонический `class_id + 1`** проекта, на нём
строятся `_shared_class_mapping` (§3.3) и разбор аннотаций. Порядок объявления менять нельзя.
...
classes:
  ...
  - {id: 41, name: "napravlenie"}
  - {id: 42, name: "vozdushnik"}
```

Парсится в список `ClassInfo(id, name)`. Полный перечень — 42 класса, от `armatura_ruchn` (1) до `vozdushnik` (42). Специальные классы: `annotation` (35) — текстовые аннотации, `output` (36) — выходы со схемы, `truba` (37) — трубы (используются для сегментации, а не детекции как оборудование), `unknow` (38) — нераспознанное оборудование (кандидат на реклассификацию по KKS-коду), `strelka` (39) — стрелки направления потока, `background` (40) — фон, `napravlenie` (41) — стрелка направления на трубе (см. [NAPRAVLENIE.md](NAPRAVLENIE.md)), `vozdushnik` (42) — воздушник.

> **`id` класса ≠ `category_id` в выгрузке CVAT.** CVAT нумерует категории **позицией метки в
> проекте** (`1 + индекс`), а не их id. Пока метки создавались в порядке `classes:`, позиция
> совпадала с `id` случайно. Соответствие восстанавливает `denormalize_coco_labels()`
> (`app/api/cvat.py`) — она сопоставляет категории **по имени** и возвращает канонические id
> до того, как что-либо запишется на диск. См. [CVAT.md §7](CVAT.md).

### 3.2.1 `display_labels`

Отображаемые названия классов (en → ru). Применяются **только на границах отображения**: имена
меток в CVAT и списки классов в клиенте. Внутри системы, в артефактах на диске и во всех
остальных конфигах класс всегда зовётся английским `name` из §3.2.

```yaml
display_labels:
  armatura_ruchn: "Арматура ручная"
  klapan_obratn: "Клапан обратный"
  # ... всего 42 записи
```

| Параметр | Тип | Default | Описание |
|----------|-----|---------|----------|
| `display_labels` | dict[str, str] | `{}` | `name` класса → отображаемое название. Пустой словарь (или отсутствие блока) = показывать английские имена, то есть прежнее поведение |

Парсится в `ProjectConfig.display_labels`; работа с ним — `app/services/class_display.py`
(`display_name`, `display_order`, `to_internal`, `sort_key`). Порядок показа — алфавит по
названию, ключ сортировки без учёта регистра и с «ё» = «е».

`ProjectLoader._parse_yaml()` проверяет блок на загрузке и **падает**, если: перевод есть не у
всех классов; перевод ведёт на несуществующий класс; два класса названы одинаково; название
совпадает с внутренним именем какого-либо класса; название длиннее 64 символов (CVAT хранит имя
метки в `SafeCharField(max_length=64)` и обрезает лишнее молча).

> **Это не копия `description` из `class_to_kks_config.yaml`.** Там техническое описание для
> KKS-привязки («Неизвестный объект — переклассифицируется по KKS»), здесь — короткая метка для
> человека («Неизвестный объект»). Синхронизировать их не нужно.

> **Менять названия после того, как CVAT-проект создан, нельзя.** `ensure_project_labels()`
> сверяет метки по имени и умеет только добавлять: правка приедет в CVAT как ещё одна метка,
...
|------------|-----------|---------------------|
| `project` + `cvat` + `classes` + `display_labels` | `ProjectConfig` | Все worker'ы, API, UI |
