...
ловит потерю целого файла (`collect_ignore`, снесённый модуль) — но именно **целого**:
запас floor'а шире, чем мелкая потеря. Пример замерен: без `shapely` из сбора молча уходят
22 теста раскладки (`importorskip`), и на машине разработки floor этого **не ловит** (738 − 22
всё ещё выше 706) — там же пропажу выдают 3 новых красных в `test_client_spec.py`; на раннере
(собирается меньше) тот же случай floor уже валит. Полагаться на один floor нельзя.
