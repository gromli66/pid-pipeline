...
**Порядок конца задачи: слияние в граф → `complete_stage` → `commit`** (с 2026-08-25, блок точечных болей, пункт 1.6). Готовность OCR клиент читает СТРОКОЙ В БД — `has_ocr_result` в `GET /api/ocr/{uid}/status` смотрит наличие `Artifact(OCR_RESULT)`. Прежний порядок (файл → артефакт → `complete_stage` → `commit` → и только потом merge) открывал окно, в котором этап уже зелёный, а текстовых блоков в графе ещё нет: оператор уходил в «Привязку подписей» по графу без текста. **Исключение из слияния больше не глотается задачей**: стадия `failed`, `error_stage='ocr'`, артефакт не закоммичен — значит клиент «готово» не покажет, а повтор возможен (сырой `ocr_result.json` остаётся на диске, труд модели не выбрасывается). ⚠ **Граница названа честно:** сама `merge_ocr_result_into_graph` (`app/services/ocr_graph_merge.py:47-90`) гасит СВОИ отказы и возвращает `0` — нет графа, нечитаемый JSON, **не удалось записать граф**; ноль при этом законен и в норме (граф ещё не собран, либо уже слито — функция идемпотентна), поэтому отличить «нечего сливать» от «запись не удалась» на стороне задачи нечем. Хорошая половина этого контракта: рядовой случай «графа ещё нет» (OCR идёт параллельно сборке) ошибкой НЕ становится. Плохая — тихий провал ЗАПИСИ по-прежнему неотличим от успеха: это A7 из ROADMAP («провал OCR-merge неотличим от успеха»), его корень в контракте функции, и он остаётся дороге.
