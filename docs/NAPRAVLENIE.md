...
**Если шаг упал** (`status=ERROR`, `error_stage="direction_classification"`), в клиенте его
несёт кнопка «Выделение труб»: своей бусины у направления нет, а перезапускает его тот же
`POST /segment`. Кнопка становится красной, клик открывает окно отчёта со строкой стадии
и запускает цепочку заново — с направления, а не с сегментации (пункт 1.12 дороги; до него
стадии не было ни в одной карте клиента, и падение не показывалось вовсе — в фолбэк-ветке
серыми оставались все тринадцать кнопок). Обе карты — `STATUS_MACHINE.md §5`.
...
**Решение Максима 2026-08-18:** граф **остаётся ненаправленным**, штамповку `direction` на все
рёбра оживлять не нужно. Поле `direction` у ребра нужно ровно в одном сценарии — **отрисовка
треугольника у узла степени 1**, когда своего направления у узла нет (узел добавлен вручную,
классификатор его не видел): `graph_to_fxml.py:1296-1315` берёт направление у инцидентного
ребра, а при его отсутствии выводит по геометрии. У всех 721 узла от классификатора
`flow_direction` есть, поэтому этот путь сегодня работает только на ручных узлах.
Критерий «два ребра с `direction`» снят. Мёртвый код (`apply_direction_rules`,
`stitch_collinear_stubs` + 6 тестов) — кандидат на снос на этапе 13; при сносе **не потерять
путь для степени 1**.
...
### 5.5 Степени direction-узлов: что норма, а что дефект
Замер по 16 схемам (`MEASUREMENTS.md §28.3`): direction-узлов **721**, из них
`pass_through: true` (степень ≥ 2) — **63 (8.7 %)**, `degree == 1` — **525**,
`degree == 0` — **133**. Ожидание плана «степень обычно 2 (вход/выход)» данными не
подтверждается.

⛔ **Поправка ВН1б (2026-08-18, `MEASUREMENTS.md §35б`):** числа выше сняты с ПОЛЯ
`degree`, а ему верить нельзя (§5.6). По фактической инцидентности в `links` на корпусе
из 19 схем (766 боксов): без единого ребра — **96**, степень 1 — **561**, степень ≥ 2 —
**109**. Разбор этих 96 — §7 чек-листа: 85 восстановимы однозначно (труба лежит на
`pipe_mask` вплотную к осевой грани, теряется на трассировке), 11 уходят оператору.

**Решение Максима 2026-08-18:**

| степень | вердикт | что делать |
|---|---|---|
| 2 | норма — труба идёт сквозь стрелку | — |
| 1 | **законно**: стрелка на терминальном участке. Разбирать не нужно | — |
| 0 | **незаконно**: труба там должна быть | чинить только там, где трубу можно восстановить со **100 % уверенностью**; где уверенности нет — **не добавлять вовсе**, оставить оператору на ручную правку |

Тихая «догадка» о трубе хуже её отсутствия: оператор видит пустое место и дорисует сам,
а неверно дорисованная труба уезжает в FXML как настоящая топология. Это входное требование
к ВН1в; сам критерий 100 % — предмет замера на схемах (ВН1б), не оценки на глаз.

### 5.6 ⛔ Поле `degree` в `graph.json` протухает после редактора
`GraphData.save()` (`ui/editors/graph_data.py:155-186`) синхронизирует только
`num_edges`/`num_nodes`/`num_isolated_nodes` и ставит клеймо `_editor_commit`; степени
узлов не пересчитываются, а созданные вручную узлы рождаются с `degree: 0`
(`:424`, `:450`) и остаются с ним навсегда. Замер 2026-08-18: расхождение поля с
инцидентностью в `links` есть ровно в тех 7 файлах корпуса из 19, где стоит клеймо
редактора (425 узлов; в файлах без клейма — ноль расхождений). Практический эффект:
37 боксов `napravlenie` числятся «сиротами» по полю, имея ребро; `pass_through`
занижен в тех же файлах, потому что считается из того же поля.

Кто читает степень стрелки — обязан считать её по рёбрам
(`tools/napravlenie_audit.py` так и делает). Починка самого поля — не задача ВН1;
это дефект редактора, кандидат в волну 10.
