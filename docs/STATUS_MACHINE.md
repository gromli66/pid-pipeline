...
(`kombu.exceptions.OperationalError`, отказ соединения ≈ 4 с), и диаграмма осталась бы
в `*ING`-статусе, которого никто не снимет: задачи нет, значит некому ни упасть в `error`, ни
дойти до конца. Кнопка своей стадии при `*ING`-статусе не нажимается, поправка
`_MANUAL_INPROGRESS` знает только четыре ручных `VALIDATING_*` — выхода без правки БД не было.
