...
Правка графа из фазы B меняет **истину** (`graph_validated.json`), от которой холст «Ручной правки» производен, — и холст по sha становится устаревшим. Пересчитать его обязано подтверждение, иначе вкладка честно предупреждает «Схема изменилась, холст будет пересобран заново», пересобирает его локальным pretransform **без раскладки**, и оператор видит ровно ту схему, на которую жаловался.
