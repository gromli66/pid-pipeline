...
4а. [Пересборка графа против фазы B](#4а-пересборка-графа-против-фазы-b)
...
Холст «Ручной правки» (`GRAPH_CANVAS` и легаси-спутник `RESIDUAL_DEFECTS`) в этой таблице **не значится** — своей стадии у него нет (его `done_status` — `GENERATING_FXML`, а он не стабильный этап). У него отдельный список `_CANVAS_ARTIFACTS` и своя граница: см. ниже.
...
|----------|----------|-------------------|
| `preserve_ocr` | Не удалять `OCR_CLEANED`, `OCR_RESULT`, `OCR_VALIDATION` — но **не** `OCR_BINDING` | Откат junction, graph, val_graph, contours |
| `preserve_contours` | Не удалять `CONTOURS_AUTO`, `CONTOURS_VALIDATED` | Откат val_graph, contours, **ocr** |

UI выставляет их по спискам `DiagramWorkspace._PRESERVE_OCR_KEYS` / `_PRESERVE_CONTOURS_KEYS`. Ключ `ocr` есть только во втором и намеренно: «Переделать OCR» обязано снести старые OCR-артефакты, но SAM2-контуры (их считают поточечно руками) к распознаванию отношения не имеют. В `preserve_ocr` этого ключа быть не должно.

⚠ `OCR_BINDING` из щадящего множества **исключён** (блок 5), а ключ `graph` убран из `_PRESERVE_CONTOURS_KEYS` — оба флага теперь стоят на кнопках, чья цель раньше `BUILT`, то есть ведёт к ПЕРЕСБОРКЕ. Что при этом гибнет и что живёт — §4а.

### Кто вообще доходит до отката

Диалог отката видят **шесть** кнопок фазы A: «Очистка рамки», «Поиск элементов», «Выделение труб», «Проверка труб», «Проверка узлов», «Сборка схемы». Конвейер там линейный, и повторный проход действительно разрушающий.

Кнопки **фазы B** — «Проверка схемы», «Контуры элемента», «Привязка подписей», «Ручная правка» — откат не предлагают вовсе: вход в пройденный этап откатом не является (решения Максима №7/№8), вкладка просто открывается на том, что уже лежит. Серверные гейты фазы B повторное сохранение и подтверждение принимают, статус при этом не двигают — см. §3 и `_SIMPLE_ALREADY_PAST` / `_BINDING_SAVE_STATUSES` / `_CONTOURS_ALREADY_PAST`. Правка при возврате штатно устаревает холст по sha (`canvas_state.is_stale`), удалять руками нечего.

Ещё три кнопки идут своими ветками: «Распознавание текста» спрашивает про **перезапуск** (это POST, а не вкладка: сырой результат сносится, счёт идёт минутами), «Проверка элементов» решает возврат внутри `_open_cvat`, «Экспорт» открывает диалог «скачать / пересобрать».

### Кто пересчитывает холст после правки в фазе B

Правка графа из фазы B меняет **истину** (`graph_validated.json`), от которой холст «Ручной правки» производен, — и холст по sha становится устаревшим. Пересчитать его обязано подтверждение.

⛔ **Клиент сам холст больше НЕ пересобирает** (блок 8 линии «Ручная правка + FXML», 2026-08-26). Раньше вкладка предупреждала «Схема изменилась, холст будет пересобран заново» и пересобирала его локальным pretransform **без раскладки** — оператор видел ровно ту схему, на которую жаловался, и правил её поверх идущего серверного пересчёта. Теперь устаревший холст даёт экран: «Раскладка пересчитывается» (когда стадия `layout` реально в `pending`/`running`) или дверь «вернитесь в „Контуры“ и подтвердите» (когда задачи нет). Подробности и таблица исходов — `UI_GUIDE.md §5.5`.

Обе двери повторного подтверждения зовут диспетчер **одинаково — без `force`**:

| дверь | эндпоинт | когда зовёт |
|---|---|---|
| «Ручная правка» → Подтвердить | `POST /graph/complete` | статус `ocr_completed` и позже (`returned_after_contours`) |
| «Проверка схемы» → Подтвердить | `POST /graph/complete-simple` | ветка `already_past`, статус `contours_validated` и позже (`_SIMPLE_LAYOUT_AFTER`) |

Почему без `force`: холст законно затирает **смена истины**, а её диспетчер видит сам по sha (решение №9). На неизменённом графе это стоит ноль работы — `ALREADY_FRESH`. С `force` метка `operator_saved` слетала бы и там, где оператор ничего не менял.

Почему раньше `contours_validated` не раскладываем: контуры ещё поменяют геометрию, и `POST /contours/complete` позовёт диспетчер сам.

Дальше дверь ведёт гейт раскладки (`ui/services/layout_gate.py`): пока новая задача считается, кнопка «Ручная правка» синяя и некликабельная, по завершении гейт открывает её **сам** — без ручного «Обновить» (пункт 1.3 дороги).

⛔ **У замка есть тупик, и он принят осознанно** (решение Максима 2026-08-26, к mefx-8). Диспетчер ставит задачу только на **смену истины**: на целом свежем холсте он отвечает `ALREADY_FRESH`, на зависшей задаче — `ALREADY_RUNNING` (`app/services/layout_policy.py`). А FAILED-строка может лежать поверх ЦЕЛОГО холста: холст и его артефакт коммитятся до `complete_stage` (`worker/tasks/layout.py`), плюс задача умеет передоставиться и переписать свою же строку. В таком состоянии повторное подтверждение «Контуров» новой стадии не заведёт, и кнопка остаётся серой; единственный работающий выход — откат до цели строго раньше `OCR_BOUND`, а он **уничтожает холст** и требует заново пройти привязку. Решение Максима: в этом состоянии в «Ручную правку» не пускать — значит подсказка обязана называть эту цену, а не только дверь (`layout_gate.LOCK_DEAD_END`, заперто тестом).

⛔ **Упавшая и просроченная раскладка кнопку ЗАПИРАЮТ** (решение Максима №9, 2026-08-26). До блока 8 оба исхода пускали оператора внутрь с модалкой «схема откроется без раскладки» — это имело смысл, пока вкладка собирала аварийный холст. Теперь собирать нечем, поэтому вместо входа в пустоту — серая кнопка, у которой в подсказке стоит причина (`error_message` стадии) и дверь повтора. Плата названа: схема, у которой раскладка падает всегда, остаётся без «Ручной правки». Запираются только ПОЛОЖИТЕЛЬНО известные исходы; «стадии нет вовсе» и «стадия без отметок времени» по-прежнему пускают — предел ожидания для них не наступает никогда, и замок стал бы вечным. ⚠ Отметка у стадии ровно одна: `/stages` отдаёт клиенту `started_at`, но **не** `created_at` (`ProcessingStageResponse`, `app/api/diagrams.py`), поэтому у задачи, стоящей в очереди, времени нет вообще, и срок ей не считается.

⚠ **Слежение после подтверждения «Контуров».** Раскладка своего статуса не имеет, её видно только стадией из `/stages`, а стадии приезжают лишь опросом `StatusProvider`. Опрос снимает себя на финальном статусе (`contours_extracted`, `ocr_bound`, `completed` — все в `_FINAL_STATUSES`), поэтому подтверждение контуров обязано его будить: `_on_contours_confirmed` зовёт `watch()` (как это делают рамка и CVAT с pains-2). Без этого о готовности пересчёта не узнают ни кнопка, ни экран ожидания вкладки — дверь не отпиралась бы без перезахода в диаграмму.

### Граница холста «Ручной правки»

`GRAPH_CANVAS` — единственный артефакт, который переживает **не всякий** откат. Правило одно, предикат один — `canvas_dies(target)`:

| цель отката | холст | `dispatch_layout(force=)` |
|---|---|---|
| `OCR_BOUND` («Привязка подписей») и позже | **живёт** — ни строки, ни файла не трогаем | `False` |
| строго раньше `OCR_BOUND` | сносится: строка **и** файл | `True` |

Почему граница проходит именно здесь: холст производен от `graph_validated.json`, и при целях раньше привязки этот источник меняется, а обратно холст не конвертируется (pretransform необратим). Возврат же **на** привязку источника не трогает — терять часы ручной раскладки не за что.

Почему `force` считается тем же предикатом: `force=True` снимает с холста метку `operator_saved` (`app/services/layout_dispatch.py`), после чего задача раскладки через 1–2 минуты перезаписывает сохранённый оператором холст (`worker/tasks/layout.py`). Сберечь строку и файл, но позвать `force`, — значит отменить всю защиту молча.

### Логика отката

1. Валидация: `target_status` должен быть в `_STAGE_ORDER`.
2. Валидация направления: цель обязана быть **строго раньше** текущего статуса. Позиция берётся из порядка объявления `DiagramStatus` — он же конвейерный, и в отличие от `_STAGE_ORDER` знает промежуточные `*_ing`. Раньше сравнение шло по `_STAGE_ORDER`, поэтому из штатного `generating_fxml` (его в списке нет) проходила **любая** цель, включая движение вперёд. Исключение — `ERROR`: он объявлен последним и позиции в конвейере не означает, из него откат разрешён куда угодно (штатный выход из тупика, §5).
3. Сбор типов артефактов: `_artifacts_to_delete(target, preserve_ocr, preserve_contours)` — этапы после target плюс холст, если он по границе гибнет.
4. `purge_artifacts()`: `DELETE FROM artifacts …` **и** удаление файлов холста с диска. Строка и файл всегда вместе — «файл без строки» опаснее удалённого: клиент артефакт не покажет (он ходит по БД), а раскладка прочитает холст с диска мимо строки и увидит в нём чужой `operator_saved`. Той же функцией пользуется четвёртый откатывающий путь — `POST /api/cvat/{uid}/reopen-bbox-validation`.
5. Установка `diagram.status = target`, очистка `error_message` и `error_stage`.
6. Если цель `CONTOURS_VALIDATED` или позже — `dispatch_layout(force=canvas_dies(target))`.

**Примечание:** прочие физические файлы на диске НЕ удаляются — только записи в БД. При повторном запуске этапа они перезаписываются. Исключение одно и названо выше: `graph_canvas.json` и `residual_defects.json` сносятся с диска вместе со строками.

**Чего откат НЕ закрывает.** Открытая вкладка «Ручной правки» о смене статуса не узнаёт (поллинг на время вкладки глушится), и её автосейв через 120 с возвращает холст обратно — гейт `/graph/canvas/save` пускает `ocr_completed` и позже. Клетка зафиксирована тестом (`tests/test_rollback_endpoint.py`), лечение — заморозка буфера вкладки на 400.

---

## 4а. Пересборка графа против фазы B

Сборка графа запускается **не один раз**: кнопка «Сборка схемы» из уже готовой
схемы, повтор после ошибки, автодиспатч из `complete_junction_validation`. Каждый
запуск переписывает `graph.json` и `graph_validated.json` целиком, а `id` узлов
между сборками **не выживают** — они порядковые `node_N` от порядка компонент
маски (`modules/graph/core/nodes.py`). Поэтому всё, что фаза B успела навесить на
узлы, после пересборки указывает в никуда либо, что хуже, на чужой элемент.

Решение Максима (2026-08-25): **на время сборки фаза B блокируется; после
пересборки привязки и контуры сбрасываются; сырой OCR живёт.**

### Гейт: пока граф собирается, писать нельзя

Предикат один — `require_graph_ready()` (`app/api/build_gate.py`). Форма
**whitelist**: чёрный список «BUILDING и раньше» пропустил бы `ERROR`, а упавшая
сборка уходит именно в него.
...
`BUILDING_GRAPH` появляется только когда воркер снимет задачу с очереди — на
CPU-only сервере это минуты ожидания.

Кто носит гейт (перечень снимается грепом `require_graph_ready` по `app/api/`):

| эндпоинт | список |
|---|---|
| `POST /contours/{uid}/extract`, `PUT /validated`, `POST /auto-accept`, `POST /complete`, `PUT /training` | `GRAPH_READY_STATUSES` |
...
украшательство: клиент фазы B приходит первым именно туда, и без признака в
отказе заслон вкладки (ниже) висел бы на пути, которого никто не потребляет.

`GET /contours/{uid}/status` и прочие читающие эндпоинты гейта **не носят** — ими
клиент опрашивает готовность. Из белого списка `POST /ocr/{uid}/start` убран
`BUILDING_GRAPH`: он первым же делом сносит `OCR_RESULT`, а сборка в этот момент
ждёт текстовые блоки, чтобы слить их в свежий граф.

### Что переживает пересборку, а что нет

Кнопки, чья цель отката раньше `BUILT`, — «Проверка узлов» (`detected_junctions`)
и «Сборка схемы» (`validated_junctions`), — ведут к пересборке. Их политика
выровнена: `preserve_ocr=True`, `preserve_contours=False`.

| артефакт | судьба | почему |
|---|---|---|
...
Открытая вкладка о смене статуса **не узнаёт** — поллинг на время вкладки
заглушён (`ui/widgets/diagram_workspace.py`), — а автосохранение молча ретраит
раз в 120 с. Сборка идёт минутами: хотя бы один тик попадёт в гейт, а как только
статус минует гейт, следующий тик зальёт **старое** поколение графа поверх
нового.

Дешёвый заслон (блок 5): отказ гейта несёт машинный признак
(`build_gate.REBUILD_REFUSAL`, копия на клиенте — `ui/tabs/save_mode.py`).
Поймав его, вкладка ставит липкую метку `save_blocked_reason`; дальше сейв
отказывается ещё до запроса, `AutoSaveService` останавливается, оператору висит
баннер «граф пересобран, закройте вкладку и откройте заново». Признак **машинный,
а не «сервер ответил 400»**: у соседних отказов правки оператора живы и лечатся
повтором.

Чего заслон **не** делает: вкладки принудительно не закрываются (контракт 1-18),
и полного закрытия окна он не даёт — для этого нужен штамп поколения
(`build_id`/sha) в самом `graph_validated.json` с отказом сейва при
несовпадении. Это кандидат в дорогу.
