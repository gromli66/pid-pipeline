...
`tools/redelivery_bench.py --leg E` (в очереди 1 сообщение против 4 до правки, 3 прогона из 3
в контейнере). **Сама гонка при этом ЖИВА как класс** и остаётся достижимой: два оператора,
два запущенных клиента, ретрай по кнопке поверх ещё не отвеченного запроса. Ниже описан
