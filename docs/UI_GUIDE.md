...
⚠ Известное отличие от §5.1, пунктом 1.x8 **не тронутое**: у необязательных заданий этой вкладки стоит дефолтный `swallow=(Exception,)`, а не `(APIError,)`, введённый пунктом 0.5 для графовой. Значит `OSError` (диск, права) на `ocr_binding` глотается молча и без флага — привязки не приезжают, вкладка об этом не говорит. Поведение заперто характеризационным тестом как факт.
