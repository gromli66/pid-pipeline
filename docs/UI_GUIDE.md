...
⚠ Остаток, пунктом 1.x12 **не тронутый**: предупреждение **не мешает** записи. Запрет слепой перезаписи ввёл 1.x9, но он живёт в `BaseGraphTab._save_graph`, а эта вкладка от базового класса не наследуется (`OcrBindingTab(AppearanceMixin, QWidget)`) — «Сохранить» после предупреждения по-прежнему затирает привязки на сервере. Поведение заперто тестом как известная граница.

**«⏳ OCR в процессе» больше не показывается на отказе, который ожиданием не лечится** (тот же пункт). `_on_download_error` повторяет загрузку 10 раз по 5 с — для «OCR ещё не готов» это правда (артефакта пока нет, сервер отвечает 404, следующая попытка его застанет), а для отказа локального диска это была ложь о работе почти на минуту, после которой оператор всё равно получал «❌ Ошибка». Граница та же, что у `swallow`: класс отказа приезжает во вкладку полем `ArtifactDownloader.last_error`, и не-`APIError` показывается сразу. ⚠ Остаток: 5xx у **обязательного** артефакта по-прежнему даёт те же 10 попыток с тем же текстом — отказ сервера со следующей попытки пройти может, и пункт этого не трогал.
