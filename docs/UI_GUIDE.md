...
1. Открыта вкладка с несохранёнными изменениями (`has_unsaved_changes()`) → вопрос «Сохранить перед выходом?» с тремя ответами. «Отмена» и неудавшееся сохранение оставляют клиент открытым (`event.ignore()`), вкладка и правки на месте.
...
- запрет стоит в `_save_graph`, потому что через него идут все три боевых входа на сервер — кнопка 💾, «Подтвердить» и автосохранение (раз в 120 с, включено по умолчанию). Ctrl+S редактора ведёт в **локальное** сохранение (`BaseGraphEditor.keyPressEvent`) и сервера не касается — это пункт 1.19.
