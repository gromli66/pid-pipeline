...
⚠ Остаток, пунктом 1.x10 **не тронутый**: отказ сервера (5xx) у `ocr_binding` по-прежнему глотается молча — привязки могли лежать на сервере, а вкладка откроется без них так же, как при законном 404. Лечится это не `swallow`, а `failure_key` (механизм §5.1); поведение заперто тестом, чтобы не сдвинулось тихо. Та же расходимость с 0.5 остаётся у вкладок масок (§5.2 PipeTab, §5.3 JunctionTab): шесть необязательных заданий с дефолтным `swallow=(Exception,)` — четыре у junction, два у pipe; из них работу оператора пишут обратно `junction_points_validated` (`_upload_points`) и `bridge_mask_validated` (`_save_masks`).
