...
| `RESIDUAL_DEFECTS` | `graph/` | `residual_defects.json` | JSON | ЛЕГАСИ — вырезано 2026-08-02, не производится |
...
`GRAPH_CANVAS` — производная от `GRAPH_VALIDATED` в холсте 1920x1080: её строит задача раскладки (`worker/tasks/layout.py`) и правит вкладка «Ручная правка»; назад в `GRAPH_VALIDATED` не конвертируется. `RESIDUAL_DEFECTS` — легаси (подсветка очагов вырезана 2026-08-02): артефакт больше не производится; на старых установках файл сносится при откате вместе с холстом (`app/api/rollback.py`), значение enum в БД остаётся (миграция `0012`).
