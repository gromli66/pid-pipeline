...
| из них `pass_through: true` | 63 (8.7 %) | **главный вопрос прогона**: норма схем или рвущаяся труба |
| из них `degree == 0` | 133 | к боксу не подошла ни одна труба — смотреть глазами |
| рёбер с `direction` | **0** | штамповка направления на рёбра в боевой сборке не вызывается (`NAPRAVLENIE.md §5.3`) |

Поэтому цель прогона — **не** «проставилось ли направление» (проставилось), а §3.2–§3.4:
цела ли труба под боксом и правильно ли она сшита.
...
- [ ] У проходного бокса `degree == 2`, `pass_through == True`; у терминального — `degree == 1`.
- [ ] **Ключевая проверка:** взять 5–10 боксов с `degree` 0 или 1 и посмотреть на схеме
      (оверлей графа / `graph_overlay.png`), действительно ли там конец ветки. Если труба
      визуально идёт сквозь бокс, а степень 0–1 — это и есть дефект ВН1в.

> `flow_direction` совпадает с `direction` из COCO для того же бокса (`ann_id`).
> Рёбер с полем `direction` сегодня **не будет** — это известное состояние (`NAPRAVLENIE.md §5.3`),
> не дефект прогона.
...
- [ ] Боксы, через которые труба идёт насквозь, дают `degree == 2` и `pass_through == True`;
      терминальные — `degree == 1`. Боксы со степенью 0 разобраны поштучно.
