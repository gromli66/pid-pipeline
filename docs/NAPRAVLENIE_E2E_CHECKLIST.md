...
**Решение Максима 2026-08-18: схема `51b339ab` — выброс, а не типичный лист** (86 сирот
из 96 при 253 стрелках — гребёнка терминальных стрелок за ручной арматурой). Цель ВН1в
считается по остальным 17 схемам корпуса, выброс разбирается отдельно:

| набор | боксов | сирот | восстановимо | оператору | цель ВН1в |
|---|---|---|---|---|---|
| 17 схем без выброса | 513 | 10 (1.9 %) | 6 | 4 (3 «скелет далеко» + 1 «нет трубы») | **10 → 4** |
| `51b339ab` (выброс) | 253 | 86 (34 %) | 79 | 7 («нет трубы») | 86 → 7, отдельным замером |

Артефакты `51b339ab` сняты 2026-08-13, но код трассировки с тех пор не менялся, поэтому
перепрогон схемы ради «а вдруг сегодня лучше» смысла не имеет — числа будут те же.
