...
⛔ **Аутентификация обязательна: `FLOWER_BASIC_AUTH=user:pass` в `.env`.** С приложением
проекта в реестре Flower лежат все боевые задачи конвейера, а `POST
/api/task/async-apply/<name>` запускает любую из них — открытый API означал бы запуск
конвейера по HTTP без авторизации. Дефолта в git нет намеренно; при пустом значении
Flower отвечает **401** и на API, и на UI (мониторинг бесполезен, но дыры нет),
а гейт честно отдаёт «судить нечем».

⛔ **Порт публикуется только на loopback** — вторым рубежом: в UI есть управление
(отозвать задачу, снять воркера). С сервера смотреть через SSH-туннель
...
Flower поднят с `-A worker.celery_app`, и окно 7200 > самой длинной задачи **и у него,
и у каждого живого воркера** (`celery -A worker.celery_app inspect conf` — окно флота
это минимум по живым потребителям, см. §1 WORKER_TASKS.md); **D** — Flower показывает
очереди и видит зондовую задачу (в очередь `default` кладётся безобидная встроенная
`celery.accumulate`, `ignore_result`). Параметры: `--queue`, `--broker`, `--container`,
`--worker-container`.
