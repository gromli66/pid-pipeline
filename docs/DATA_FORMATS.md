...
- **KKS** только из `graph.bindings`; кегль — не формула, а `KKS_FONT_SIZE`, и с 2026-08-25 это не своё число, а `TEXT_STYLES['kks'].size` = **18.0** (см. §«Подписи: одна таблица стилей»).
- **Контуры** не вливаются на экспорте — они уже в холсте (см. §4 «Связь с графом»).
- **Мосты «— | —»** — геометрией, как в старом пути. Терминальные рёбра пропускаются; пины (`pin_source`/`pin_target`) не экспортируются; непривязанные `text_blocks` → `<Text>` Tahoma 18 Bold (см. ниже).
...
⚠ **Шрифт фиксируется ТОЛЬКО в FXML** (решение №7): `<Font name="Tahoma" size="18.0"/>` плюс `style="-fx-font-weight: bold;"`. ⛔ Атрибут `name` несёт **семейство, а не начертание**: полное имя начертания («Tahoma Bold») JavaFX молча подменяет на System — баг JDK-8089450, и файл при этом остаётся валидным. Проверено рендером в JavaFX 18 (движок SceneBuilder), замер `MEASUREMENTS §MEFX3`: после `applyCss` у всех 48 подписей листа `Tahoma 18.0 Bold`, промах центра по X упал с **33.85 px до 1.58 px**, площадь подписи против площади рамки — с 5.4× до 1.5×. Шрифты в клиент не бандлим: у экрана и у выгрузки общий только **кегль**, семейство на экране прежнее.
...
⛔ **Подписи диаметра в таблице НЕТ, и в FXML она не печатается** (решение Максима 2026-08-25, доработка mefx-3). В файл от диаметра уходит только `strokeWidth` плюс XML-комментарий. Держит это не «мы её не пишем», а общее исключение по привязке: текст-блок, у которого есть привязка, в `<Text>` не идёт (`bound_block_ids`), а подпись диаметра — это блок, привязанный к РЕБРУ. Следствие: паритета с файлом у неё нет, поэтому файловому кеглю 18 она **не подчиняется** — кегль у неё свой, внутренний, одинаковый во всех вкладках клиента (`base_graph_editor._create_edge_label`). Новой эмиссии не заводить: она напечатала бы подпись дважды (второй раз — из привязки, волна 4).
