...
| `kksFontSize` | `TEXT_STYLES['kks'].size` = **18.0** — константа, одна на оба пути выгрузки (было `min(w,h)*0.35` с ограничениями; см. §«Подписи: одна таблица стилей») |
...
- **KKS** только из `graph.bindings`; кегль — не формула, а `KKS_FONT_SIZE`, и с 2026-08-25 это не своё число, а `TEXT_STYLES['kks'].size` = **18.0** (см. §«Подписи: одна таблица стилей»). С 2026-08-26 подпись привязанного блока печатается **своим `<Text>`**, а на контроле гасится (`kksVisible="false"`); атрибут `kks=` остаётся — связь узел↔код не трогается. `kksVisible="true"` остаётся там, где печатать нечего: заглушка датчика `fff` и ручной KKS (`nodekks_*`) текст-блока не имеют.
- **Контуры** не вливаются на экспорте — они уже в холсте (см. §4 «Связь с графом»).
- **Мосты «— | —»** — геометрией, как в старом пути. Терминальные рёбра пропускаются; пины (`pin_source`/`pin_target`) не экспортируются; `text_blocks` → `<Text>` Tahoma 18 обычного начертания (см. ниже) — и свободные, и привязанные к узлу, каждый в рамке своего блока; привязанные к ребру (диаметр) по-прежнему не печатаются.
...
### Подписи: одна таблица стилей (блок 3 линии «Ручная правка + FXML», 2026-08-25)

Кегль подписи, **уходящей в файл**, — фиксированное число **18**, одно на все её виды и на оба пути выгрузки (решение Максима 2026-08-25 №3). Таблица — `modules/graph_to_fxml.TEXT_STYLES` (`text_block` / `kks`), её же читает редактор у подписи текст-блока (`ocr_layer_mixin._LABEL_PX`). ⛔ Подписи диаметра в таблице нет — см. врезку в конце раздела. Было четыре числа: `_TEXT_FONT_SIZE = 40.0` у `<Text>`, `KKS_FONT_SIZE = 10.0` у холстового пути и две доли от размера бокса (`0.35·min(w,h)`, `0.4·min(w,h)`) у растрового — на одном листе `d74eb9f1` последние давали 5.1 и 7.2, отсюда жалоба «на одном листе подписи разного размера».

⛔ **Коэффициентов в FXML не осталось.** Пятый, `_TEXT_CHAR_W_FACTOR = 0.55`, был ОЦЕНКОЙ ширины строки: из неё считался `layoutX`, и подпись уезжала по X тем сильнее, чем длиннее текст. Он **удалён**, а не перекалиброван: выравнивание отдано формату — `layoutX` = левый край рамки, `wrappingWidth` = её ширина, дальше работает `textAlignment="CENTER"` (вариант Б решения Максима). ⚠ `textAlignment` стоял в выгрузке и раньше, но для однострочного `<Text>` **без `wrappingWidth` он мёртв** — потому и жила оценка. У вертикального блока (поворот −90°) роль ширины играет высота рамки, а `layoutY` садится на её нижнюю грань.

⚠ **Шрифт фиксируется ТОЛЬКО в FXML** (решение №7): `<Font name="Tahoma" size="18.0"/>`; начертание — **обычное у любой подписи**, привязанной и свободной (решение Максима 2026-08-26; до него уходил `style="-fx-font-weight: bold;"`, замеры ниже сняты на нём). ⛔ Атрибут `name` несёт **семейство, а не начертание**: полное имя начертания («Tahoma Bold») JavaFX молча подменяет на System — баг JDK-8089450, и файл при этом остаётся валидным. Проверено рендером в JavaFX 18 (движок SceneBuilder), замер `MEASUREMENTS §MEFX3`: после `applyCss` у всех 48 подписей листа `Tahoma 18.0 Bold`, промах центра по X упал с **33.85 px до 1.58 px**, площадь подписи против площади рамки — с 5.4× до 1.5×. Шрифты в клиент не бандлим: у экрана и у выгрузки общий только **кегль**, семейство на экране прежнее.

⚠ **Границы охвата.** У скиновой KKS-подписи семейство и начертание атрибутами не задаются вовсе — в файл уходит только `kksFontSize` (развязка с атрибутом `kks=` — волна 4, не здесь).

⛔ **Подписи диаметра в таблице НЕТ, и в FXML она не печатается** (решение Максима 2026-08-25, доработка mefx-3). В файл от диаметра уходит только `strokeWidth` плюс XML-комментарий. Держит это исключение по привязке к РЕБРУ: в холстовом пути в `<Text>` не идут только такие блоки (`edge_bound_block_ids`), в старом — любые привязанные (`bound_block_ids`). Следствие: паритета с файлом у неё нет, поэтому файловому кеглю 18 она **не подчиняется** — кегль у неё свой, внутренний, одинаковый во всех вкладках клиента (`base_graph_editor._create_edge_label`). Новой эмиссии не заводить: она напечатала бы подпись дважды (второй раз — из привязки, волна 4).
...
## Остаток авто-раскладки (`graph/residual_defects.json`) — легаси

Артефакт `ArtifactType.RESIDUAL_DEFECTS`. **Легаси:** подсветка очагов
вырезана 2026-08-02 — артефакт больше никем не производится и UI его не
читает. На старых установках файл сносится при откате вместе с холстом
(`app/api/rollback.py`). Формат ниже — для чтения файлов, оставшихся от
прежних установок.
...
- `canvas_sha` — проекция холста, для которого посчитан остаток (сверка с
  холстом делалась вкладкой «Ручная правка» до вырезания 2026-08-02).
