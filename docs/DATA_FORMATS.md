...
Кегль подписи — **фиксированное число 18**, одно на все виды подписи и на оба пути выгрузки (решение Максима 2026-08-25 №3). Таблица — `modules/graph_to_fxml.TEXT_STYLES` (`text_block` / `kks` / `diameter`), её же читает редактор (`base_graph_editor._create_edge_label`, `ocr_layer_mixin._LABEL_PX`). Было четыре числа: `_TEXT_FONT_SIZE = 40.0` у `<Text>`, `KKS_FONT_SIZE = 10.0` у холстового пути и две доли от размера бокса (`0.35·min(w,h)`, `0.4·min(w,h)`) у растрового — на одном листе `d74eb9f1` последние давали 5.1 и 7.2, отсюда жалоба «на одном листе подписи разного размера».
...
⚠ **Границы охвата.** У скиновой KKS-подписи семейство и начертание атрибутами не задаются вовсе — в файл уходит только `kksFontSize` (развязка с атрибутом `kks=` — волна 4, не здесь). Подпись **диаметра** в FXML не печатается совсем (только `strokeWidth` + XML-комментарий); её кегль из таблицы читает только редактор, новой эмиссии блок не заводит.
