...
### 9.5 Legacy: KksBinder (TextBinder удалён)

Файл: `modules/kks_binding/binder.py`. `KksBinder` — KKS код → equipment node; заменён `UnifiedBinder`, сохранён для обратной совместимости.

⛔ **`modules/text_binding/binder.py` (TextBinder) УДАЛЁН 26.08.2026.** Его `propagate_diameters` был старым правилом потока: диаметр свободно проходил через любой `type == equipment` при любой степени (замер на корпусе 24 схем: 2152 протечки из 4745 проходов), развилки судились сравнением ориентаций H/V вместо угла (ломалось на 342 диагоналях), а классовое правило было одно — `perehod`. Заменён **правилом линии** — `modules/binding/diameter_lines.py`: Ду с метки оператора красит всю ЛИНИЮ ребра и ничего кроме, таблица проходимых/стоп-классов лежит в YAML проекта (секция `diameter_lines`). `bind_diameters` ушёл вместе с ним: авто-привязка Ø выключена решением заказчика, диаметр ставит только оператор.
