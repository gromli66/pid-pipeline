...
@pytest.mark.xfail(strict=True, raises=AssertionError,
                   reason="баг «H раньше V» (seating.reseat_edge:189-197): "
                          "вырожденный Y-диапазон коннектора ставит H-замок, "
                          "V-ветка недостижима; конец садится на боковую "
                          "грань [80, 140] вместо нижней [80, 120] "
                          "(нарисовано 20.0px при зазоре 0.5px). Чинится Э2")
def test_h_before_v_vertical_link_must_seat_on_bottom_face():
    e = _h_before_v_case()
    # коннектор жёстко в центроиде — это верно и сейчас, и после Э2
...
@pytest.mark.xfail(strict=True, raises=AssertionError,
                   reason="П1 нарушается багом «H раньше V»: коннектор в "
                          "0.5px под гранью соседа даёт сегмент 20.0px под "
                          "1.4° к нижней грани (вдоль неё). Чинится Э2")
def test_p1_perpendicular_exit_with_degenerate_pair():
    """Та же синтетика + вырожденная пара из теста 4: свойство П1 обязано
    держаться и на ней (после Э2 конец сядет на нижнюю грань)."""
    g = _clean_synthetic()
