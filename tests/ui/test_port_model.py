...
def test_port_switches_once_and_does_not_flap(qapp, tmp_path):
    ...
    При проходе НАД осью соседа между портами легально живёт фаза строгой
    прямой (слабина прямизны — «прямые как сейчас», инвариант §5 спеки):
    последовательность состояний обязана быть монотонной R+ S* T+ —
    правый порт, прямая, верхний порт — без единого возврата."""
    g = _graph_box_conn_right()
    _assert_canonical(g)
    ed = _editor(qapp, tmp_path, g)
    e = ed.model.find_edge_data(ed.model.edge_key("b", "c"))
    ...
    for fx, fy in frames:
        ...
        if off == (40.0, 0.0):
            ...
            # между портами — только фаза строгой V-прямой на оси соседа
            assert e["source_point"][1] == pytest.approx(400.0, abs=0.5), \
                f"вне портов конец обязан быть на строгой прямой: {off}"
            states.append("S")
    ed.end_drag_node()

    compact = [s for i, s in enumerate(states)
               if i == 0 or states[i - 1] != s]
    assert compact == ["R", "S", "T"], \
