...
(`base_graph_editor.py:_create_edge_label`), хотя правил её единственный
жест — Ctrl+2ЛКМ в «ОКР привязке», а в FXML её нет вовсе.
⛔ С 26.08.2026 этот жест СНЯТ (решение Максима: правки Ду в «Ручной правке»
не будет вовсе — она пишет в холст, а `.prtx` читает `graph_validated`).
Подпись осталась: она показывает Ду, пришедший с этапа привязки. Решение: поведение то же, что у блоков текста
...
def test_правка_диаметра_из_ручной_правки_снята(ed):
    """⛔ Решение Максима 26.08.2026: функционала Ду в «Ручной правке» нет.

    Раньше Ctrl+2ЛКМ по ребру открывал диалог правки диаметра. Он писал Ду в
    ХОЛСТ, а холст уходит в `graph_canvas`, тогда как `.prtx` собирается из
    `graph_validated` (`app/api/graph.py:562`) и назад холст в него не пишется
    (`app/models/artifact.py:61`). То есть правка до расчётной схемы не доезжала
    вообще: оператор правил, видел число на холсте и получал в САПФИР заводские
    0.3 м = честный Ду300. Ду правится в одном месте — «Привязка текста».

    Сторож на ДВЕ стороны: жеста нет, а ПОДПИСЬ на месте — снята правка,
    а не отображение.
    """
    editor, g = ed
    assert not hasattr(editor, "_open_diameter_edit_dialog")
    assert not hasattr(editor, "_propagate_all_diameters")
    ...
    assert e == before, "жест изменил ребро, хотя правка Ду снята"
    assert len(_visible_labels(editor)) == DIAM_COUNT
