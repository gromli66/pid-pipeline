...
# `skeletonizing_final` — вторая такая клетка на время блока болей pains-1:
# клиент её уже понимает (карта правится в [ui]-коммите), а писатель переезжает
# на неё следующим, [сма]-коммитом того же блока. Порядок именно такой: обратный
# оставил бы значение, которого клиент не знает, — тупик из этого же пункта.
CLIENT_ONLY = {"validating_graph", "skeletonizing_final"}
