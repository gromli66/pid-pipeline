...
# Кусок скелета короче — шум скелетизации, а не труба. Приоритет — меньше
# грязи в графе: оператору дешевле дорисовать недостающее, чем искать лишнее.
MIN_LEFTOVER_LENGTH = 50
...
def trace_edges_v3(skeleton: np.ndarray,
    ...
    Args:
        ...
        buried_connectors: label_id стыков, сидящих на маске оборудования
            (доля >= BURIED_MIN_OVERLAP) — такой стык не перехватывает
            контакт у элемента
    ...
    for fragment in _leftover_fragments(skeleton, visited, MIN_LEFTOVER_LENGTH):
        ...
        edges.append({
            ...
            'color': None,
            # провенанс для стадии 5: подобранный кусок должен прижиться обоими
            # концами (узел или врезка в трубу), иначе он идёт под нож
            'leftover': True,
        })
        edge_counter += 1
        if debug:
...
def trace_single_edge_v3(start_node_id: str,
    ...
    while True:
        ...
        # Тупик. Ветка после развилки — откатываемся и пробуем другое
        # направление; откат помечает ветку visited, из графа она уходит.
        # Порог SPUR_LENGTH (2026-08-24) оставлял длинные тупиковые ветки как
        # «настоящие свободные концы», но оператор таких не рисует ни на одной
        # схеме корпуса, а искать их по листу дорого. Решение заказчика
        # 2026-08-25: лишний узел дороже недостающего — откат снова
        # безусловный, как до порога; часть настоящих веток теряется
        # (замер: покрытие ручных рёбер −19 на 6 схемах), это принято.
