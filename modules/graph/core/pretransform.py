...
def apply_fixed_sizes(graph, adj):
    ...
    У узла ровно ОДНА форма, и выбирается она по наличию класса в таблице
    (решение заказчика 2026-07-28, §3.7 плана AUTO_LAYOUT_INTEGRATION.md):
    есть в словаре — как в словаре, нет в словаре — как в реальности. Поэтому
    словарному классу контур СНИМАЕТСЯ: раньше подменялся только bbox, и у
    узла с контуром в одном файле оказывались две разные формы —
    прямоугольник из таблицы и полигон от сегментации. Видно это прямо в
    «Ручной правке»: скины по умолчанию выключены. В FXML контур скинового
    узла всё равно игнорируется в пользу скина, так что снятие ничего не
    теряет.
    """
    for n in graph.get("nodes", []):
        cn = n.get("class_name")
...
def point_on_polygon(seg, px, py, tol=0.5):
    """Лежит ли точка на границе контура (в пределах tol px).

    Нужна тем, кто «доводит» конец ребра до контура: посадка (`seating`) уже
    сажает его туда сама, и повторная доводка лучом ИЗ ЦЕНТРОИДА сбивает
    ортогональную трубу в диагональ. Замер на боевом c2f79462: у всех 13 рёбер
    деаэратора конец лежит на контуре с расстоянием 0.000 px, а редактор
    пересчитывал его заново и рисовал звезду из центра.

    Без shapely: модуль тянут UI и `text_import`, а в `requirements/ui.txt`
    его нет.
    """
...
def seat_edge_endpoints(graph):
    """Посадка концов рёбер модулем `seating` — одним на всю систему (§3.4).

    Этим путём идёт то, что ВИДИТ ОПЕРАТОР: холст, собранный `pretransform`
    для «Ручной правки», и выход раскладки (она сажает концы тем же модулем).
    Раньше здесь была своя реализация, а у раскладки своя — надмножество
    (замок на общую ось для строгих H/V, станции на контуре, жёсткое правило
    коннектора), и одно и то же ребро рисовалось ими по-разному.

    Импорт внутри функции: `seating` сам берёт отсюда `FIXED_SIZES` и
    `project_ray_to_polygon`, на уровне модуля вышел бы цикл.

    Возвращает число сдвинутых концов (для статистики pretransform).
    """
    from .seating import reseat_all_endpoints

    before = [(e.get("source_point"), e.get("target_point"))
              for e in _edges(graph)]
    reseat_all_endpoints(graph)
    fixed = 0
    for (sp0, tp0), e in zip(before, _edges(graph)):
        fixed += (e.get("source_point") != sp0) + (e.get("target_point") != tp0)
    return fixed


def reproject_edge_endpoints(graph):
    """Проекция концов на границу формы — ВХОД РАСКЛАДКИ, не отрисовка.

    Почему не заменена на `seat_edge_endpoints` вместе со всем остальным
    (замер 2026-07-28, Э6): этой функцией строится `orig` — до-раскладочная
    геометрия, которую лестница ходов использует как ИСТОЧНИК ОСИ «ОРИГИНАЛ»
    (§4.5 SOLUTION.md). У ребра к крупному блоку центроид лежит глубоко
    внутри, и по центроидам получается диагональ, хотя труба входит в бок
    строго по оси; спасают именно эти, «сырые» точки подключения. Посадка
    `seating` их идеализирует — замок на общую ось стирает ровно тот сигнал
    направления, ради которого источник и заведён.

    Цена подмены замерена на корпусе: дефектов **20 -> 64**, время a6d28736
    20 -> 135 с. Расстановка при этом не меняется (defects_before те же), то
    есть разница целиком в слое раздвигания.

    Отличие от `seating`, важное при сверке: здесь конец ребра дублируется в
    ТЕРМИНАЛЬНУЮ точку `path`/`waypoints`, тогда как и FXML-экспорт, и
    редактор строят полилинию как `start + waypoints + end` (waypoints —
    промежуточные). На текущих данных это мертво — ни одного ребра с
    waypoints, — но затирало бы первую промежуточную точку маршрута.
    """
...
def pretransform(graph, image_hw=None):
    ...
    # на границу итогового (фикс-размер + сдвиг declust). Это путь ОТРИСОВКИ
    # (холст «Ручной правки»), поэтому посадка общая с раскладкой — §3.4.
    stats["endpoints_reprojected"] = seat_edge_endpoints(g)
    return g, transform, stats
