...
Замер по 19 холстам заказчика (tools/smooth_bench.py повторяет):
  ...
  * зигзаги с ОДНОЙ ступенькой: 60 шт., смещение медиана 12px, 75% <= 14,
    максимум 22 — ровно зона «чуть сдвинуть». Отсюда BUDGET = 16.
...
# ── пороги ─────────────────────────────────────────────────────────────
BUDGET = 16.0        # px: максимум «чуть сдвинуть» (75% ступенек корпуса;
                     # 22 закрыло бы все — константа вынесена ради подбора
                     # глазами заказчика)
