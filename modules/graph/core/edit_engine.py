...
def seat_end(node, other_node, edge_data, role, cur, ref_x, ref_y,
    ...
    ЯКОРЬ ВХОДА (решение заказчика 2026-08-02 «нужен классический обход, но
    с фиксированным входом») — ПЕРВЕЕ ВСЕГО: если оператор сам поставил вход
    этого ребра (`ports.pinned_port` — порт с владельцем), конец сидит ровно
    там. Якорь не участвует в конкурсе портов, не отбрасывается гистерезисом
    при уводе оси и работает на ЛЮБОМ типе узла, включая контурные (прежняя
    «святость ручного порта» жила только в рамочной ветке и терялась на
    полигонах). Форму трубы это не фиксирует — маршрут по-прежнему считает
    роутер, обходя препятствия.
    ...
    """
    pin = port_model.pinned_port(node, edge_data)
    if pin is not None:
        return pin[0], pin[1]
