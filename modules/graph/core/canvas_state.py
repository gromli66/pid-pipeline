...
LAYOUT_CODE_VERSION = "5"   # 5 (2026-07-31, Э-A): пины роутинга в порты
                            # (`layout/ports.py` + avoid_router): концы
                            # роутимых рёбер садятся в ПОРТ узла (центр
                            # грани/прямого участка контура, судья тот же,
                            # что у drag в редакторе), а не в канон-луч —
                            # «ребро на угол» у пары бокс -> крупный контур
                            # уходит. Холсты версии 4 обязаны устареть.
                            # 4 (2026-07-31, Э7-b): этап роутинга — обход
                            # чужих форм libavoid после раздвигания
                            # (`layout/avoid_router.py`): транзитные рёбра
                            # получают waypoints, боксы уходят с чужих
                            # магистралей. Холсты версии 3 — без обходов,
                            # обязаны устареть.
                            # 3 (2026-07-30, Э2): выбор оси посадки — от
