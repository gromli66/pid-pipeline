...
- valueVisible="false" для всех контролов кроме FunctionControl (убирает артефакт "0")
...
def generate_fxml_control(node, geometry: SkinGeometry, node_id: str,
    ...
    # Скрыть дефолтное значение "0" у контролов (убирает артефакт "0").
    # ИСКЛЮЧЕНИЕ: FunctionControl — у него valueVisible="false" скрывает
    # не только текст значения, но и сам графический скин
    # (FLOWMETER, FILTER, ARROW и др. пропадают целиком).
    if control_class != 'FunctionControl':
