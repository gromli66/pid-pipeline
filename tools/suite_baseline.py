...
#
# ⛔ **ПОТОЛОК ПРИВЯЗАН К РАЗМЕРУ НАБОРА, А НАБОР РАСТЁТ. КАЛИБРОВАТЬ ПО ФОРМУЛЕ,
# А НЕ ПО ПАМЯТИ.** Первая редакция числа (900 с) была снята при **3036**
# собранных и через пять дней стала врать: к 2026-08-25 собрано **4284**, и два
# прогона подряд легли на 97–98 % с вердиктом `unjudged=hang`. Дампа
# `faulthandler` во втором не было вовсе — то есть «вис» пришёлся не на тест,
# а на сам потолок; стенд честно сказал «судить нечем», но сказать ему было
# нечего не из-за кода, а из-за собственной калибровки. Цена ошибки
# несимметрична: слишком КОРОТКИЙ потолок превращает годный прогон в
# «судить нечем» и обесточивает гейт для всех сессий разом, слишком ДЛИННЫЙ
# стоит лишь ожидания в редком настоящем висе.
#
# ФОРМУЛА КАЛИБРОВКИ (применять при КАЖДОМ заметном росте набора):
#
#     RUN_TIMEOUT >= 2 × T(N),   где T(N) — ЗДОРОВЫЙ прогон САМОГО СТЕНДА
#                                целиком (сбор ПЛЮС прогон) при текущем N
#
# ⛔ **T(N) МЕРИТСЯ СТЕНДОМ, А НЕ СУММОЙ ДВУХ `pytest` — иначе завышает почти
# вдвое.** Замерено 2026-08-25 в один день, оба числа настоящие:
#
#   * ПРЯМОЙ замер, один прогон стенда при N = 3935 → **760 с** (exit 0);
#   * КОСВЕННЫЙ, суммой `pytest tests --ignore=tests/ui` (98.6 с) и
#     `pytest tests/ui` (1055.5 с) при N = 4284 → 1154 с, с надбавкой ~1270 с.
#
# Выпадающее наблюдение названо ПЕРВЫМ, а не выброшено: разошлись они не на 9 %
# разницы в N, а вчетверо больше, и причин две — два отдельных процесса вместо
# одного и загруженная машина (косвенный замер шёл сразу после другого полного
# прогона). Тот, кто возьмёт косвенное число за T(N), получит требование
# 1905 с и решит, будто 1800 нарушает собственную формулу. Не нарушает:
# по прямому замеру 1800 / 760 = ×2.37, а с линейным пересчётом на N = 4284
# (≈ 827 с) — ×2.18. Правило «больше ДВОЙНОГО» из пункта 1-47 остаётся в силе,
# ослаблять его не пришлось.
#
# Верхняя граница у потолка тоже есть, и это НЕ он сам: бюджет задания CI за
# вычетом прочих шагов (установка зависимостей, сбор, `lint_gate`,
# воспроизводимость раскладки). При задании в 45 минут (2700 с) значение 1800
# оставляет им ~900 с.
#
# ⛔ **ЧИСЛА ТРИ, И ОНИ СВЯЗАНЫ ПОРЯДКОМ — ДВИГАЯ ОДНО, СВЕРЯЙ ДВА ДРУГИХ:**
#
#     TEST_TIMEOUT (120) < RUN_TIMEOUT (1800) < задание CI (2700)
#
# Левое неравенство держит имя кадра в захвате (иначе стенд вернётся раньше
# дампа, заперто тестом); правое держит третий исход пункта 1-47 живым в CI —
# сравняйся потолок стенда с бюджетом задания, и GitHub убьёт задание раньше,
# чем стенд отдаст `exit 2`, то есть «судить нечем» снова станет неотличимо от
# оборванного задания. Ровно это и случилось 2026-08-25 при `timeout-minutes:
# 30`, и правка этого пункта подняла бюджет до 45 (`.github/workflows/tests.yml`,
# там же обоснование).
#
# Историческая калибровка (§116.1, §116.8), сохранена для сверки тренда: при
# N = 3036 здоровый прогон целиком — 385 с, два оборванных крахом замера входа
# дошли до 97 % за 304 с и 265 с, соседние сессии видели 311–404 с.
# TEST_TIMEOUT: один тест длиной 120 с — это ~10 % всего прогона при N = 4284
# (было 40 % при N = 3036), то есть окно осталось с запасом «медленный, но
# живой»; поднимать его вместе с RUN_TIMEOUT не надо — оно про ОДИН тест,
# а не про набор, и от роста N не зависит.
TEST_TIMEOUT = 120
RUN_TIMEOUT = 1800
COLLECT_TIMEOUT = 300
