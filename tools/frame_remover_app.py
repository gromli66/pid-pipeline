"""
Batch Frame Remover — удаление рамок и штампов с рендеров чертежей.

Два инструмента:
  🔲 Полигон (внешнее) — обвести внутреннюю часть чертежа,
     всё СНАРУЖИ полигона заливается фоном.
  ⬛ Бокс (внутреннее) — нарисовать прямоугольник,
     всё ВНУТРИ бокса заливается фоном.

Workflow:
  1. Полигоном убрать внешнюю рамку
  2. Боксом убрать штамп / таблицу
  3. Ctrl+S — сохранить в renders_clean/ и перейти дальше
  4. Ctrl+D — пропустить

Запуск:
    python frame_remover_app.py
"""
...
IMAGES_DIR = Path(r"C:\Users\Maksim\Desktop\after3project\png")
OUTPUT_DIR = Path(r"C:\Users\Maksim\Desktop\after3project\png\not_frame")

IMAGE_EXTS = {".png", ".jpg", ".jpeg", ".bmp", ".tif", ".tiff"}


def collect_queue() -> list[tuple[Path, Path]]:
    """(input_path, output_path), пропуская уже обработанные."""
...
class BatchFrameRemover(QMainWindow):
    ...
    def __init__(self):
        ...
        self._editor = FrameRemoverView()
        self._editor.status_callback = self._on_status

        self.setWindowTitle("Batch Frame Remover")
        ...
        if not self._queue:
            QMessageBox.information(self, "Готово", "Нет файлов для обработки.")
            sys.exit(0)
    ...
    def _setup_toolbar(self):
        ...
        btn_restart_poly = QPushButton("🔄 Новый полигон")
        btn_restart_poly.clicked.connect(self._restart_polygon)
        tb.addWidget(btn_restart_poly)
        ...
        btn_skip = QPushButton("⏭ Skip (Ctrl+D)")
        ...
        act_skip = QAction(self)
        act_skip.setShortcut(QKeySequence("Ctrl+D"))
    ...
    def _load_current(self):
        img_path, out_path = self._queue[self._index]
        ok = self._editor.load_image(str(img_path))
        if not ok:
        ...
        self.setWindowTitle(
            f"[{self._index + 1}/{total}] {img_path.name} — Batch Frame Remover"
        )
        self.statusbar.showMessage(
            f"{img_path.name}  →  {OUTPUT_DIR.name}/{img_path.name}"
        )
        # Активировать текущий инструмент
        self._set_tool(self._editor.tool)

    def _save_and_next(self):
        if self._index >= len(self._queue):
            return
        _, out_path = self._queue[self._index]
        self._editor.save_image(str(out_path))
        self.statusbar.showMessage(
            f"Сохранено: {OUTPUT_DIR.name}/{out_path.name}", 2000
        )
        self._next()
    ...
    def _restart_polygon(self):
        """Начать новый полигон (для повторного применения)."""
