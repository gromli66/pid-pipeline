...
- pair_bench.py (ДН4, пункт 0.11) — стенд диффов «до/после оператора»: семь пар артефактов
  ...
  Эталон **версии 2** (пункт 1-45) держит рядом с числами `inputs` — отпечаток входа каждой
  пары (sha256 обеих её сторон, по байтам: половина пар не json). Пропавшая пара и
  сменившийся вход — «судить нечем» (2), а не «рост правок» (1); плоский эталон без
  `inputs` не судит вовсе до пересъёма
- napravlenie_audit.py (ВН1б) — приёмка стрелок направления по артефактам: §4 чек-листа
  `docs/NAPRAVLENIE_E2E_CHECKLIST.md` без консольных вставок плюс разбор боксов без рёбер
  на «восстановимо однозначно» / «оператору». Степень узла считает по `links`, а не по полю
  `degree` (после редактора оно протухшее). Эталона нет, `--check` судит два инвариантных
  критерия приёмки; данные — локальный `storage/diagrams/`, в CI идут только юнит-тесты
  логики. Числа замера — `MEASUREMENTS.md` §35
