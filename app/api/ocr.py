...
# (`worker/tasks/ocr.py`). Решением Максима 2026-08-25 кнопка на этом статусе
# ПОГАШЕНА (`_light_ocr_button`); этот отказ — страховка прямых путей (API,
# гонка тика) и обязан оставаться ПО-РУССКИМ, а не английской строкой про
# 'validated_junctions'.
_OCR_START_STATUSES = (
