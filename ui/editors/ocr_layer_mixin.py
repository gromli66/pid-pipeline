...
class OcrLayerMixin:
    ...
    def _ocr_push_snapshot(self, description: str):
        """Снять точку возврата под OCR-команду.

        ⛔ Живое превью панели «Размеры» снимается ДО снимка (пункт 1.x8,
        механизм из 1.4/1.5). Состояние «ОКР привязка» и инструмент «Размеры» —
        независимые оси и включаются одновременно, поэтому OCR-команда достижима
        при живом превью: Delete, вставка блоков, правка текста/KKS двойным
        кликом и асинхронный приход результата распознавания. Превью мутирует
        МОДЕЛЬ мимо стека команд, значит `SnapshotCommand._before` снялся бы
        вместе с ним — и Ctrl+Z вернул бы превью, а «Сохранить» увезло бы его
        на сервер (замер §73б). Поздним откатом это не лечится: `_before`
        отравлен необратимо.

        Место выбрано ровно здесь, потому что это ЕДИНСТВЕННАЯ точка построения
        точек возврата всего слоя — в отличие от `advanced_graph_editor`, где
        команды строятся в ~25 местах и 1.4 пришлось закрывать четыре пути
        поштучно (§56.22). Граница 1.4 (И5) при этом соблюдена по построению:
        все зовущие делают ранний выход «нечего делать» ДО этого вызова, то есть
        команда, точки возврата не построившая, превью не снимает.
        """
