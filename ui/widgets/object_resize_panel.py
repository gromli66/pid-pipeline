...
class ObjectResizePanel(QFrame):
    ...
    def __init__(self, parent):
        ...
        self._bridge_spin.setToolTip(
            ...
            "⚠ Единственная настройка оформления, которая УХОДИТ В ВЫГРУЗКУ:\n"
            "остальные регуляторы меняют только картинку на экране, а этот —\n"
            "и разрывы в готовом FXML. Включите «Скины», чтобы увидеть их\n"
            "на холсте такими же, как в файле."
        )
