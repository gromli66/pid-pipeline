...
class ContourTab(BaseGraphTab):
    ...
    def _save_graph(self) -> bool:
        ...
        try:
            ...
            # Гейт пересборки — не «сохраните ещё раз»: сохранять некуда,
            # контуры сидят на узлах прошлого поколения (блок 5).
            banner = self.rebuild_banner(exc)
            # По таймеру — строкой, а не модалкой посреди работы (1-38).
            if self._save_interactive:
                QMessageBox.warning(
                    self, "Схема пересобрана" if banner else "Контуры не сохранены",
                    banner or (
                ...
                self._refuse_save(
                    banner
                    or f"⚠️ Автосохранение: граф сохранён, контуры — нет ({exc})")
            if banner:
