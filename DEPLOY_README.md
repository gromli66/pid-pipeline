...
- **OCR и контуры (после обновления `deploy`).** OCR по умолчанию **включён** (`configs/projects/thermohydraulics/thermohydraulics.yaml` → `ocr.enabled: true` — так и на `deploy`, и на `dev`): этап отрабатывает в обычной последовательности контуры → OCR → привязка. Выключить — `false` + `docker compose restart api worker_ocr`; тогда этап пропускается и бусины авто-проматываются в `OCR_BOUND`. Контуры SAM2 **не считаются автоматически** — распознавание запускается во вкладке контуров кнопками «Распознать выбранные» (Shift+ЛКМ по узлам) / «Распознать все»; на CPU тяжёлый этап, удобнее точечно.
